...
namespace DystirManager.Views
{
    ...
    public partial class SetMatchTimeAndPeriodPage : ContentPage
    {
        ...
        public SetMatchTimeAndPeriodPage(Match selectedLiveMatch)
        {
            _isMatchTime = true;
            _match = selectedLiveMatch;
            InitializeComponent();
            Populate(selectedLiveMatch);
            BindingContext = selectedLiveMatch;
        }

        public SetMatchTimeAndPeriodPage(Match selectedLiveMatch, EventOfMatch eventOfMatch)
        {
            _isMatchTime = false;
            _match = selectedLiveMatch;
        ...
        private void Populate(Match selectedLiveMatch)
        {
            if (_isMatchTime)
            {
            ...
            SetTime(0, 0);
            StatusListView.ItemsSource = selectedLiveMatch.Statuses?.Where(x => x.StatusID > 0 && x.StatusID < 13).ToList();
        }
