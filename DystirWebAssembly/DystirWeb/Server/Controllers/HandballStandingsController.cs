...
namespace DystirWeb.Controllers
{
    ...
    [ApiController]
    public class HandballStandingsController : ControllerBase
    {
        private DystirDBContext _dbContext;
        private List<HandballTeams> _teamsList;
        private IQueryable<HandballMatches> _matchesList;

        public HandballStandingsController(DystirDBContext dystirDBContext)
        {
        ...
        {
            DateTime date = new DateTime(DateTime.Now.Year -1 , 1, 1);
            _teamsList = _dbContext.HandballTeams?.ToList();
            _matchesList = _dbContext.HandballMatches?.Where(x => x.MatchTypeId != null
                    && x.MatchActivation != 1
                    && x.MatchActivation != 2
                    && x.Time > date);
            List<Standing> standingsList = new List<Standing>();
            var competititionNamesArray = new string[] { "Burn menn" };
        ...
        internal List<TeamStanding> GetStandings(string competitionName)
        {
            ...
            try
            {
                List<HandballMatches> matches = _matchesList?.Where(x => x.MatchTypeName == competitionName && /*(x.StatusId == 13 || x.StatusId == 12)&& */ (x.RoundId < 1000)).ToList();
                foreach (HandballMatches match in matches)
                {
        ...
        private void CalculatePoints(TeamStanding teamStanding, HandballMatches match, int? mainTeamScore, int? opponentTeamScore)
        {
        ...
        private void ReducePenaltiesPoint(List<TeamStanding> teamStandingsList)
        {
            ...
            {
                new Tuple<string, string, int>("2. deild", "B68", 2)
            };
        ...
        {
            if (teamStanding.CompetitionName == "Burn menn")
            {
